...
import { getAllPosts } from "../services/service.js";
import { Link } from "react-router-dom";

const InternalChosenPosts = ({ category, id }) => {
  const posts = useMemo(() => getAllPosts(), []);

  const filtered = useMemo(() => {
    return posts.filter((p) => {
      return p.category === category && p.id !== Number(id);
    });
  }, [category, posts, id]);

  const chosenPosts = filtered.length > 3 ? filtered.slice(0, 3) : filtered;
...
export default InternalChosenPosts;
